...
namespace OrderForm {
    public partial class FormMain : Form {
        ...
        private void InitOrders() {
            Order order = new Order(1, new Customer("1", "Chris"), new List<OrderDetail>());
            order.AddItem(new OrderDetail(1, new Product("1", "note", 10.0), 100));
            order.AddItem(new OrderDetail(2, new Product("2", "hole", 20.0), 20));
            order.AddItem(new OrderDetail(3, new Product("4", "break", 100.0), 2));
            orderService.AddOrder(order);
            Order order2 = new Order(2, new Customer("2", "Liu"), new List<OrderDetail>());
            order2.AddItem(new OrderDetail(1, new Product("3", "slide", 30.0), 5));
            order2.AddItem(new OrderDetail(2, new Product("4", "break", 100.0), 2));
        ...
        private void btnDelete_Click(object sender, EventArgs e) {
            ...
            DialogResult result = MessageBox.Show($"Are you sure to delete order {order.OrderId}?", "Delete?", MessageBoxButtons.YesNo);
